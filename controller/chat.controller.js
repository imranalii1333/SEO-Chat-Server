...
const createGroupChat = asyncHandler(async (req, res) => {
    const allowedEmails = ["ia@gmail.com", "ia2@gmail.com", "ia3@gmail.com"];
    ...
    users.push(req.user);
    const allowedUsers = users.filter(user => allowedEmails.includes(user.email));

    if (allowedUsers.length !== 3) {
        return res.status(403).json({ error: "You do not have permission to create a group chat." });
    }
